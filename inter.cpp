...
int main() {
    ...
    while (l <= N) {
        ...
        if (NORM <= TOL) {
            ...
            std::cout << "Values of w[i][j]:" << std::endl;
            for (int i = 0; i < n + 1; i++) {
                for (int j = 1; j < m + 1; j++) {
                    std::cout << "w[" << i << "][" << j << "] = " << std::setw(10) << w[i][j] << " ";
                    }
                    std::cout << std::endl;
                    }
        ...
        // Step 20: Save the data to a file
        ...
        // Step 21: Visualize the data using gnuplot
        FILE* gnuplotPipe = popen("gnuplot -persist", "w");
        if (gnuplotPipe) {
            // Set up Gnuplot commands to create a 3D plot
            fprintf(gnuplotPipe, "set view 60,30\n");
            fprintf(gnuplotPipe, "set xlabel 'x'\n");
            fprintf(gnuplotPipe, "set ylabel 'y'\n");
            fprintf(gnuplotPipe, "set zlabel 'w'\n");
            fprintf(gnuplotPipe, "splot 'data.txt' with pm3d\n");
            fflush(gnuplotPipe);
            pclose(gnuplotPipe);
        } else {
            std::cout << "Unable to open Gnuplot pipe." << std::endl;
        }
